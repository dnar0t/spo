 ...
 * - Вызывает entity.recall(actorId) для перехода статуса
...
export class RecallTimesheetUseCase {
  ...
  async execute(
    ...
    // 2. Вызываем доменный метод отзыва таймшита
    timesheet.recall(actorId);
