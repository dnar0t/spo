...
function getStageCounts(stageId, details) {
    if (!details)
        return null;
    return details[stageId] || null;
}
function formatStageResult(counts) {
    if (!counts)
        return "";
    const parts = [];
    parts.push(`${counts.processed} обработано`);
    if (counts.changes > 0)
        parts.push(`${counts.changes} изменений`);
    if (counts.errors > 0)
        parts.push(`${counts.errors} ошибок`);
    return parts.join(", ");
}
...
function SyncDialog({ open, onOpenChange }) {
    const [syncRunId, setSyncRunId] = (0, react_1.useState)(null);
    ...
    return (<dialog_1.Dialog open={open} onOpenChange={(o) => {
            if (!o && !isRunning)
                onOpenChange(o);
        }}>
      <dialog_1.DialogContent className="max-w-lg">
        <dialog_1.DialogHeader>
          <dialog_1.DialogTitle className="flex items-center gap-2">
            {isRunning && <lucide_react_1.Loader2 className="h-5 w-5 animate-spin"/>}
        ...
        <div className="space-y-2 py-2">
          {STAGES.map((s, i) => {
            ...
            return (<div key={s.id} className={(0, utils_1.cn)('flex items-center gap-2 text-sm px-2 py-2 rounded', isActive && 'bg-primary/10 text-primary font-medium', isPast && 'text-muted-foreground', !isPast && !isActive && 'text-muted-foreground/60')}>
                {isActive ? (<lucide_react_1.Loader2 className="h-4 w-4 animate-spin shrink-0 text-primary"/>) : isPast ? (<lucide_react_1.CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-600"/>) : (<div className="h-4 w-4 shrink-0 rounded-full border-2 border-muted-foreground/30"/>)}
                <div className="flex-1 min-w-0">
                  <span>{s.label}</span>
                  {isPast && resultText && (<span className="text-[11px] text-muted-foreground ml-1">— {resultText}</span>)}
                  {isActive && (<div className="text-[11px] text-muted-foreground mt-0.5">
                      {s.description}
                      {s.id === 'workItems' && status && (<span className="ml-1">
                          ({status.updatedCount} записей обработано)
                        </span>)}
                      {s.id !== 'workItems' && status && status.totalIssues > 0 && (<span className="ml-1">
                          ({totalProcessed} из {status.totalIssues} обработано)
                        </span>)}
                    </div>)}
                </div>
                {isPast && counts && counts.errors > 0 && (<badge_1.Badge variant="outline" className="text-[10px] bg-destructive/5 text-destructive border-destructive/20">
                    {counts.errors} ошиб.
                  </badge_1.Badge>)}
              </div>);
        })}
        ...
        {(isRunning || isDone) && (<div className="space-y-1">
            ...
            <progress_1.Progress value={progressPercent} className="h-2"/>
          </div>)}

        
        {isDone && (<div className="grid grid-cols-3 gap-2 p-3 rounded-md bg-muted/30 border border-border text-center text-xs">
            <div>
        ...
        {isFailed && (<div className="flex items-start gap-2 p-3 rounded-md bg-destructive/5 border border-destructive/20 text-sm text-destructive">
        ...
        {cancelled && (<div className="flex items-start gap-2 p-3 rounded-md bg-muted border border-border text-sm text-muted-foreground">
            <lucide_react_1.Ban className="h-4 w-4 shrink-0 mt-0.5"/>
            <div>
        ...
        <dialog_1.DialogFooter>
          {isRunning ? (<button_1.Button variant="destructive" size="sm" onClick={handleCancel}>
              <lucide_react_1.XCircle className="h-4 w-4 mr-1"/> Отмена
            </button_1.Button>) : (<button_1.Button size="sm" onClick={handleClose} className="bg-primary hover:bg-primary-hover">
              OK
            </button_1.Button>)}
        </dialog_1.DialogFooter>
