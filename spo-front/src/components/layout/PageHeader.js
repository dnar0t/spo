...
function PageHeader({ title, description, breadcrumbs, actions }) {
    return (<div className="border-b border-border bg-gradient-header">
      <div className="px-6 py-3">
        {breadcrumbs && breadcrumbs.length > 0 && (<nav className="text-[11px] text-muted-foreground mb-1 flex items-center gap-1.5">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          {actions && (<div className="flex items-center gap-2 shrink-0 ml-auto flex-nowrap">
              {actions}
            </div>)}
        </div>
