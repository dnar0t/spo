...
**Версия контекста:** 1.2  
**Дата обновления:** 2026-05-01  
**Статус:** проект развёрнут на сервере srv-dev2 (100.64.100.111), портал доступен по http://100.64.100.111  
...
- ✅ **Part 5 — YouTrack and Hub Integration — завершён (контроллер вынесен в use cases, добавлены guards)**
  ...
  - YouTrack Use Cases: проверка статуса, синхронизация, тест соединения — вынесены из контроллера
  - Guards: JwtAuthGuard + RolesGuard закрывают все эндпоинты (CR-04)
  - YouTrackModule подключён в AppModule
  - `npm run build` — без ошибок (85 файлов)

- ✅ **Part 6 — Sprint Planning — полностью завершён (Transactional Outbox добавлен)**
  ...
  - **Use Cases**: CreatePeriod, UpdatePeriod, GetPeriods, GetPeriodDetail, GetBacklog (с фильтрацией, деревом, пагинацией), GetCapacity, AssignTask, UnassignTask, FixPlan (с PlanFixedEvent + Transactional Outbox), TransitionPeriod
  ...
  - **Transactional Outbox**: OutboxService, OutboxProcessor, FixPlanUseCase использует транзакционную отправку через OutboxService (PlanFixedEvent)
  - PlanningAppModule зарегистрирован в AppModule
  - `npm run build` — без ошибок (85 файлов)

- ✅ **Верификация и аудит (29.04.2026) — проведён**
  - Аудит проекта: [`DOC/Verifications/verification_290426_1.md`](../../Verifications/verification_290426_1.md)
  - План исправлений: [`DOC/ARC/plan_verification_290426_1.md`](plan_verification_290426_1.md)
  - Исправлено ~95% замечаний (все согласованные)
  - Подробнее: [Результаты аудита](#результаты-аудита-29042026)

- Проект опубликован на GitHub: https://github.com/dnar0t/spo

- ✅ **Part 3 — Security, Auth, RBAC/ABAC, Audit — core завершён (RBAC исправлен, CORS/секреты усилены)**
  ...
  - **Application Layer**: LoginUseCase (AD bind → user lookup → JWT → audit) — роли загружаются из БД, RefreshTokenUseCase (ротация с SHA-256 хешем), LogoutUseCase (revoke sessions), GetCurrentUserUseCase
  ...
  - **Guards**: JwtAuthGuard (Bearer token validation + загрузка ролей из БД), RolesGuard + @Roles() декоратор — роли унифицированы через ROLES константы (@spo/shared)
  - **Controller**: POST /api/auth/login, POST /api/auth/refresh, POST /api/auth/logout, GET /api/auth/me, POST /api/auth/test-ldap
  - **Security**: Helmet HTTP headers, rate limit on login, пароль не логируется/не сохраняется, JWT только sub/login/sessionId (роли из БД); CORS настроен через allowlist; JWT/Encryption secrets — fail-fast вместо dev fallback
...
- ✅ **Part 7 — Period Workflow, Fact Loading, Report Recalculation — core завершён (финансовые группировки по system/project реализованы)**
...
- ✅ **Part 6 — Sprint Planning — полностью завершён**
  - Перенос readiness из предыдущего месяца (CarryOverReadinessUseCase)
  - Изменение фиксированного плана директором с audit log (ModifyFixedPlanUseCase + PlanModifiedEvent)
  - Handler выгрузки плана в YouTrack (ExportPlanToYouTrackUseCase + EventBusService)
  - Endpoints: POST /api/planning/periods/:id/carry-over-readiness, PUT /api/planning/periods/:id/modify-fixed-plan
  - Plan.md обновлён, context.md обновлён

- ✅ **Part 10 — Notifications and Export — полностью завершён**
  - SMTP settings, notification templates/queue/history
  - Excel/PDF/JSON export services
  - Async export jobs with download links
  - Export controller (7 endpoints)

### 4.2 Что ещё не сделано

- ✅ Реализован handler выгрузки плана в YouTrack
- ✅ Настроен CI/CD (`.github/workflows/ci.yml`)
- ✅ Написаны тесты: 97 тестов (RBAC, ABAC, freeze, snapshot immutability, fix-plan+outbox, YouTrack guards, smoke)
- ✅ Health endpoint расширен: `/health/live` и `/health/ready` (с проверкой БД)
- ✅ ExportController: добавлены `@Roles` на все методы
- ✅ YouTrack guards: все эндпоинты закрыты JwtAuthGuard + RolesGuard
...
- **Основание:** [`DOC/Verifications/verification_290426_1.md`](../../Verifications/verification_290426_1.md)
- **План исправлений:** [`DOC/ARC/plan_verification_290426_1.md`](plan_verification_290426_1.md)
- **Всего замечаний:** 20 критичных + 8 средней важности
- **Исправлено:** ~95% замечаний (все согласованные)
- **Основные исправления:**
  - RBAC/авторизация полностью исправлена (роли загружаются в JwtAuthGuard, унифицирован регистр, YouTrackController закрыт guards)
  - Security: CORS allowlist, fail-fast для JWT/Encryption secrets
  - Согласование frontend/backend: убраны mock-данные, API_BASE_URL в env, spo-front в workspace
  - Транзакционный Outbox: реализован OutboxService + OutboxProcessor, FixPlanUseCase использует
  - Snapshot: workItems, issues, hierarchy собираются из БД
  - Freeze финансов: isFrozen=true в persistence
  - Финансовые группировки: реальные данные вместо заглушек UNKNOWN/Stub
  - Shared контракт: Role, PeriodState и др. вынесены в @spo/shared
  - CI/CD: создан workflow (.github/workflows/ci.yml)
  - Тесты: 97 тестов (RBAC, ABAC, freeze, snapshot immutability, fix-plan+outbox, YouTrack guards, smoke)
  - Health endpoint: расширен с /health/live и /health/ready (с проверкой БД)
  - ExportController: добавлены @Roles на все методы
- **Отложено** (не блокирует, требуется инфраструктура):
...
| 14.0 | 2026-04-27 | Team Lead | Part 9 (Frontend) — Страница закрытия периода интегрирована с реальным API: создан usePeriodClose hook (React Query: readiness checklist, close, reopen, snapshot status/data, statistics), PeriodClose.tsx переведён с моков на API-вызовы (checklist готовности, close/reopen диалоги, просмотр snapshot, история периодов). Build — без ошибок. |

| 15.0 | 2026-04-27 | Team Lead | Part 4 (Frontend) — Административные страницы интегрированы с реальным API: создан useAdmin hook (React Query: users CRUD, roles, manager, dictionaries, audit log, sessions, sensitive changes, planning settings, integrations), Users.tsx, Audit.tsx, Settings.tsx переведены с моков на API-вызовы. Build — без ошибок. |

| 16.0 | 2026-04-27 | Team Lead | Part 10 (Frontend) — Страница настроек включает таб интеграций (SMTP, YouTrack, LDAP) с управлением через API. Notification settings skeleton и export actions доступны через Settings.tsx и интеграционные карточки. Build — без ошибок. |
...
| 17.0 | 2026-04-27 | Team Lead | Part 6 — Реализован перенос readiness из предыдущего месяца (CarryOverReadinessUseCase): находит предыдущий период, копирует readinessPercent задач с совпадающим issueNumber. Реализовано изменение фиксированного плана директором с audit log (ModifyFixedPlanUseCase): проверка состояния периода (не CLOSED, не PLANNING), обновление часов задачи через updatePlannedHours, создание новой версии SprintPlan, аудит через PeriodTransition.forAudit(), событие PlanModifiedEvent. Добавлены endpoints: POST /api/planning/periods/:id/carry-over-readiness, PUT /api/planning/periods/:id/modify-fixed-plan. Добавлен метод findPreviousPeriod в ReportingPeriodRepository и PrismaReportingPeriodRepository. Проект компилируется — 398 файлов без ошибок. plan.md обновлён. |

| 18.0 | 2026-04-27 | Team Lead | Обновление context.md после Parts 3, 4, 7, 8, 9. Реализован handler экспорта плана в YouTrack (ExportPlanToYouTrackUseCase + EventBusService + YouTrackExportServiceImpl): при PlanFixedEvent обновляет custom field "Спринт" (94-77), устанавливает assignee и добавляет тег "SPO planned" через YouTrack API. Создан EventBusService (in-process pub/sub). Создан SharedModule (глобальный). Зарегистрирован IntegrationAppModule. Проект компилируется — 404 файла без ошибок. plan.md обновлён. |

| 19.0 | 2026-04-29 | AI Agent | Проведён аудит проекта (`verification_290426_1.md`), составлен план исправлений (`plan_verification_290426_1.md`). Исправлено ~95% замечаний. Обновлён context.md: версия 1.1, добавлены результаты аудита, обновлён статус завершённых частей (CI/CD, тесты, health endpoint, ExportController roles, YouTrack guards). |
